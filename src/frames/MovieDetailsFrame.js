...
export const MovieDetailsFrame = ({ route }) => {
  const { movieId } = route.params;

  const { movies } = useContext(MovieContext);

  const movieToDisplay = useMemo(
    ...
    <ScrollView>
      <ScreenContainer>
        {movieToDisplay ? (
          <>
            {Boolean(
              movieToDisplay.Poster && PosterImages[movieToDisplay.Poster]
            ) && (
              <Image
                source={PosterImages[movieToDisplay.Poster]}
                style={{ width: "100%" }}
              />
            )}
            ...
            <View style={{ paddingHorizontal: 15, paddingTop: 15 }}>
              <Text>
                IMDB: {movieToDisplay.imdbRating}{" "}
                {movieToDisplay.imdbVotes &&
                  `(${movieToDisplay.imdbVotes} votes)`}
              </Text>
            </View>
